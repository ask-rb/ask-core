...
Patch numbers advance by exactly one per release: `0.12.0` → `0.12.1` →
`0.12.2`. Never skip or jump patch numbers (`0.12.0` → `0.12.3` is wrong),
...
gemchain update ask-core <new-version> --dry-run
gemchain update ask-core <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-core.gemspec`).
...
- Bug fix only: `0.12.0` → `0.12.1`.
- Several bug fixes ship together: one release, `0.12.0` → `0.12.1` (never
  `0.12.3`).
- New backwards-compatible API: `0.12.1` → `0.13.0`.
- Breaking change while pre-1.0: `0.13.0` → `0.14.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.12.1` →
  `0.12.2`, via the gemchain cascade — never hand-published.
